...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_Add_Card] = "images\\Menu_AddCard.jpg";
...
void Output::DrawLadder(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	else if (GetCellStartY(fromCell) == GetCellStartY(toCell)) {
		this->PrintMessage("end cell and start cell can not be in the same Cell");
		return;
	}
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	//	if((fromCell.GetCellNum()-toCell.GetCellNum())>0 && (fromCell.GetCellNum()-toCell.GetCellNum())%11==0)
	...
	int x[4] = { x12,+x12 - (xChange / 2),x12,x12 + (xChange / 2) };
	int y[4] = { y1,y1 + (yChange / 2),y1 + yChange,y1 + (yChange / 2) };
